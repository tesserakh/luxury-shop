...
class ShopSpider(scrapy.Spider):
    ...
    def parse_product(self, response):
        ...
        try:
            ...
            try:
                item['price'] = product.css('p.price > span.woocommerce-Price-amount > bdi::text').get().strip()
                item['discount'] = None
            except:
                item['price'] = product.css('p.price > del > span.woocommerce-Price-amount > bdi::text').get().strip()
                item['discount'] = product.css('p.price > ins > span.woocommerce-Price-amount > bdi::text').get().strip()
            try:
            ...
            item['availability'] = product.css('div.product-stock > span:nth-child(2)::text').get().strip()
